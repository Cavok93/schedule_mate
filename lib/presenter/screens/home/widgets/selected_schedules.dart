...
import 'package:today_mate_clean/presenter/widgets/create_card.dart';
...
class _SelecteSchedulesSheetState extends State<SelecteSchedulesSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    _bottomPadding =
        Platform.isAndroid ? 16.0 : MediaQuery.of(context).padding.bottom;
              ...
              children: [
                CalendarSelectedDateSelector((selectedDate) {
                  return Container(
                    ...
                    padding:
                        const EdgeInsets.only(left: 16.0, top: 6, bottom: 6),
                    alignment: Alignment.centerLeft,
                    child: FittedBox(
                      fit: BoxFit.fitHeight,
                      child: Text(
                        KRTime(selectedDate).format().toString(),
  ...
  Widget _buildListView(List<Schedule> selectedSchedules) {
    return Column(
      children: [
        Expanded(
          child: ListView.separated(
            ...
            itemCount: selectedSchedules.length + 1,
            itemBuilder: (context, index) {
              if (selectedSchedules.isEmpty && index == 0) {
                return CreateCard(
                  callBack: () {},
                );
              } else if (index == selectedSchedules.length) {
                return Padding(
              ...
              final schedules = selectedSchedules[index];
              return ScheduleCard(
                  schedule: schedules,
                  callBack: (id) {
